...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        grouped_xyz = grouping_operation(xyz_trans, idx)  # (B, 3, npoint, nsample)
        if self.relative_xyz:
            grouped_xyz1 = grouped_xyz - query_xyz.transpose(1, 2).unsqueeze(-1)  # relative position
            if self.normalize_dp:
                grouped_xyz1 /= self.radius

        sinta1 = torch.sum(grouped_xyz1, dim=-1, keepdim=False)

        groupted_distance = torch.norm(grouped_xyz1,p=2,dim=1,keepdim=True)
        groupted_alfa = torch.atan2(grouped_xyz1[:,1,:,:].unsqueeze(1),grouped_xyz1[:,0,:,:].unsqueeze(1))
        groupted_beta = torch.atan2(grouped_xyz1[:, 2, :, :].unsqueeze(1), torch.norm(grouped_xyz1[:, :2, :, :],p=2,dim=1,keepdim=True))
        grouped_xyz = torch.cat([grouped_xyz1,groupted_distance,groupted_alfa,groupted_beta],dim=1)
        sinta2 = torch.mean(torch.cat([groupted_distance,groupted_alfa,groupted_beta],dim=1), dim=-1, keepdim=False)
        sinta = torch.cat([sinta1,sinta2],dim=1)
        grouped_features = grouping_operation(features, idx) if features is not None else None
        return grouped_xyz1, grouped_features
