...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        self.SDAE = SDAE(channels[0], channels[-1], flag=1)
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_type = feature_type
        self.grouper = create_grouper(group_args)
        self.grouper1 = create_grouper(group_args1)
        group_args1['nsample'] = 8
        ...
        create_conv1 = create_convblock2d
        convs = []
        ...
        convs11 = []
        for i in range(len(channels) - 1):
            convs11.append(create_conv1(4, channels[i+1],
                                     norm_args=norm_args if not is_head else None,
                                     act_args=None if i == len(channels) - 2
                                                      and (self.use_res or is_head) else act_args,
                                     **conv_args)
        ...
        self.convs11 = nn.Sequential(*convs11)

        convs12 = []
        for i in range(len(channels) - 1):
            convs12.append(create_conv1(4, channels[i+1],
                                       norm_args=norm_args if not is_head else None,
                                       act_args=None if i == len(channels) - 2
                                                        and (self.use_res or is_head) else act_args,
                                       **conv_args)
        ...
        self.convs12 = nn.Sequential(*convs12)

        convs13 = []
        for i in range(len(channels) - 1):
            convs13.append(create_conv1(3, channels[i + 1],
                                        norm_args=norm_args if not is_head else None,
                                        act_args=None if i == len(channels) - 2
                                                         and (self.use_res or is_head) else act_args,
                                        **conv_args)
        ...
        self.convs13 = nn.Sequential(*convs13)

        convs14 = []
        for i in range(len(channels) - 1):
            convs14.append(create_conv1(3, channels[i + 1],
                                         norm_args=norm_args if not is_head else None,
                                         act_args=None if i == len(channels) - 2
                                                          and (self.use_res or is_head) else act_args,
                                         **conv_args)
        ...
        self.convs14 = nn.Sequential(*convs14)

        convs1 = []
        for i in range(len(channels) - 1):
            convs1.append(create_conv1(channels[i+1],channels[i+1],
                                       norm_args=norm_args if not is_head else None,
                                       act_args=None if i == len(channels) - 2
                                                        and (self.use_res or is_head) else act_args,
                                       **conv_args)
        ...
        convs6 = []
        for i in range(len(channels) - 1):
            convs6.append(create_conv1(4, 16,
                                      norm_args=norm_args if not is_head else None,
                                      act_args=None if i == len(channels) - 2
                                                       and (self.use_res or is_head) else act_args,
                                      **conv_args)
        ...
        self.convs6 = nn.Sequential(*convs6)

        convs7 = []
        for i in range(len(channels) - 1):
            convs7.append(create_conv1(4, 16,
                                      norm_args=norm_args if not is_head else None,
                                      act_args=None if i == len(channels) - 2
                                                       and (self.use_res or is_head) else act_args,
                                      **conv_args)
        ...
        self.convs7 = nn.Sequential(*convs7)
        self.sample_fn1 = furthest_point_sample
        self.sample_fn2 = random_sample
        self.pool1 = lambda x: torch.mean(x, dim=-1, keepdim=False)
    ...
    def forward(self, pf):
        p, f, i= pf
        if i == 0:
            dp, fjj = self.grouper1(p, p, f)
            fjj = self.convs6(fjj)
            ff = self.convs7(f.unsqueeze(-1))
            df1 = fjj - ff
            f_distance1 = torch.sum(df1, dim=1, keepdim=False)
            f_distance1 = self.softmax(f_distance1)
            f_distance1 = torch.std(f_distance1, dim=-1, unbiased=False)
            idxx1 = f_distance1.topk(1024, dim=-1)[1]
            h_f1 = torch.gather(f.permute(0,2,1), dim=1, index=idxx1.unsqueeze(-1).repeat(1,1,f.shape[1]))
            h_p1 = torch.gather(p, dim=1, index=idxx1.unsqueeze(-1).repeat(1,1,p.shape[2]))

            idx2 = self.sample_fn1(f.permute(0, 2, 1), 1024).long()
            s_f1 = torch.gather(f.permute(0, 2, 1), 1, idx2.unsqueeze(-1).expand(-1, -1, f.shape[1]))
            s_p1 = torch.gather(p, 1, idx2.unsqueeze(-1).expand(-1, -1, p.shape[-1]))

            h_f2 = self.convs11(h_f1.unsqueeze(-1).permute(0,2,1,3))
            s_f2 = self.convs12(s_f1.unsqueeze(-1).permute(0,2,3,1))

            g_f1 = h_f2 - s_f2
            g_f2 = self.convs1(g_f1)
            g_f3 = self.pool1(g_f2)

            h_p2 = self.convs13(h_p1.unsqueeze(-1).permute(0, 2, 1, 3))
            s_p2 = self.convs14(s_p1.unsqueeze(-1).permute(0, 2, 3, 1))

            g_p2 = h_p2 - s_p2
            g_p3 = self.pool1(g_p2)

            g_3 = g_f3 + g_p3
            g_6 = []
            for g in range(g_3.shape[0]):
                idxx12 = [idxx1[g, :]]
                zero1 = torch.zeros([f.shape[2], g_3.shape[1]]).cuda()
                g_4 = g_3[0, :, :].permute(1, 0)
                g_5 = zero1.index_put_(idxx12, g_4).unsqueeze(0)
                g_6.append(g_5)
...
class SetAbstractioncls(nn.Module):
    ...
    def __init__(self,
        ...
        if self.all_aggr:
            ...
            self.convs = SDAE(channels[0] - 3, channels[-1], self.flag)
        if not is_head:
            if self.all_aggr:
                group_args.nsample = None
                group_args.radius = None
...
class SDAE(nn.Module):
    def __init__(self, channel, out_channel, group_args={'NAME': 'ballquery', 'radius': 0.1, 'nsample': 16}, flag=0):
        super().__init__()
        self.channel = channel
        self.grouper = create_grouper(group_args)
        self.mlp1 = nn.Sequential(nn.Conv2d(channel, channel//2, 1, 1, bias=False), nn.BatchNorm2d(channel//2),
                                           nn.ReLU(inplace=True))
        self.mlp2 = nn.Sequential(nn.Conv2d(channel, channel//2, 1, 1, bias=False), nn.BatchNorm2d(channel//2),
                                           nn.ReLU(inplace=True))
        self.mlp3 = nn.Sequential(nn.Conv2d(channel, channel//2, 1, 1, bias=False), nn.BatchNorm2d(channel//2),
                                           nn.ReLU(inplace=True))
        self.flag = flag
        if flag == 0:
        ...
        self.result = nn.Sequential(nn.Conv1d(channel//2, out_channel, 1, 1, bias=False), nn.BatchNorm1d(out_channel))
    ...
    def forward(self, position, identity, y):  # y b,d,g,1
        ...
        f2 = torch.mul(v, s2)
        f = torch.sum(f2, dim=-1, keepdim=False)
        f = self.result(f)
...
class PointVectorEncoder(nn.Module):
    ...
    def forward_seg_feat(self, p0, f0=None):
        ...
        for i in range(0, len(self.encoder)):
            # if i == 0:
            #     _p, _f, idx = self.encoder[i]([p[-1], f[-1], i])
            # else:
            _p, _f = self.encoder[i]([p[-1], f[-1], i])
            p.append(_p)
            f.append(_f)
        return p, f
